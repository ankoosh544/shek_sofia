...
class CoreController implements ICoreController {
  ...
  CoreController() {
    ...
    GetIt.instance
        .registerLazySingleton<ICoreController>(() => CoreController());
    ...
    notificationManager!.notificationReceived
        .listen((event) => NotificationManager_NotificationReceived);
    bleService!.onSampleReceived.listen((event) => BleService_OnSampleReceived);
    bleService!.onDeviceDisconnected
        .listen((event) => BleService_OnDeviceDisconnected);
    if (resolver != null) {
  ...
  Future<void> StopCharacteristicWatchAsync() async {
    try {
      ...
      bleService?.onCharacteristicUpdated
          ?.listen((event) => BleService_OnCharacteristicUpdated(this, event));
  ...
  void BleService_OnCharacteristicUpdated(
      Object sender, BLECharacteristicEventArgs e) async {
    try {
      ...
      // invio evento
      if (onCharacteristicUpdated != null) onCharacteristicUpdated(this, null);
    } catch (ex) {
      await showDialog(
        context: context,
        builder: (context) => AlertDialog(
  ...
  void EmitNotifications(BLEDevice device) {
    if (IsFloor(device)) {
      if (!isInForeground) {
        int secondiPassati =
            ((DateTime.now().microsecondsSinceEpoch - tickAttuali) ~/ 1000000);
        debugPrint('secondi:');
        debugPrint(secondiPassati.toString());
        if (secondiPassati > IntervalloAvvisoVicinoAscensore ||
            PrimaConnessioneDevice) {
          PrimaConnessioneDevice = false;
          Vibration.vibrate();
          notificationManager.sendNotification(
              'Soffia', Res.AppResources.YouAreNearTheElevator);
          audioService.beep();
  ...
  void NotificationManager_NotificationReceived(BuildContext context) async {
    if (await authService!.isLoggedAsync()) {
      await Navigator.pushNamedAndRemoveUntil(
          context, '/CommandPage', (route) => false);
    } else {
      await Navigator.pushNamedAndRemoveUntil(
